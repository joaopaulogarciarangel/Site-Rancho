...
export default function GarcomApp() {
  ...
  const removerDoMenu = (idFinal: string) => {
    setCarrinho((prev) => {
      ...
      const ultimo = itensDesseTipo[itensDesseTipo.length - 1];
      if (ultimo.quantidade > 1) {
        return prev.map(i => i.uid === ultimo.uid ? { ...i, quantidade: i.quantidade - 1 } : i);
      }
      return prev.filter(i => i.uid !== ultimo.uid);
    });
  };

  const alterarQuantidadeUID = (uid: string, delta: number) => {
    setCarrinho((prev) =>
      prev.map(item => item.uid === uid ? { ...item, quantidade: item.quantidade + delta } : item)
  ...
  const enviarParaCozinhaEBar = async () => {
    ...
    setComandasPorMesa((prev) => {
      ...
      carrinho.forEach(itemCarrinho => {
        const idx = novaComanda.findIndex(i => i.idProduto === itemCarrinho.idProduto && i.observacao === itemCarrinho.observacao);
        if (idx >= 0) {
          novaComanda[idx] = { ...novaComanda[idx], quantidade: novaComanda[idx].quantidade + itemCarrinho.quantidade };
        } else {
  ...
  const liberarMesaCaixa = async (numero: number, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!window.confirm(`Confirmar pagamento e liberação da Mesa ${numero}?`)) return;
    try {
      ...
      const { error: erroHistorico } = await supabase.from('vendas_historico').insert([{
        mesa: numero, itens: itensConsumidos, valor_total: totalFinal, forma_pagamento: pagamentoEscolhido
      }]);
      ...
      try {
        ...
        const { data: estoqueAtual, error: erroBusca } = await supabase
          .from('estoque').select('id, nome_produto, quantidade_atual').in('nome_produto', nomesConsumidos);
      ...
      setComandasPorMesa(prev => { const n = { ...prev }; delete n[numero]; return n; });
      setMesasAguardandoPagamento(prev => prev.filter(m => m !== numero));
      setPagamentosPorMesa(prev => { const n = { ...prev }; delete n[numero]; return n; });
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 pb-28">
                    ...
                    return (
                      <div key={opcao.idOpcao} className="flex justify-between items-center bg-gray-50 p-2.5 rounded-xl border border-gray-200">
                        ...
                        <div className="flex items-center gap-3 bg-white rounded-lg p-1 shadow-sm border border-gray-300">
                          <button onClick={() => handleBotaoMenos(produto, opcao.idOpcao)} className={`p-1.5 rounded ${quantidade > 0 ? 'text-gray-900' : 'text-gray-400'}`} disabled={quantidade === 0}><Minus className="w-5 h-5" /></button>
                          <span className="font-black w-6 text-center text-black text-lg">{quantidade}</span>
                          <button onClick={() => handleBotaoMais(produto, opcao.idOpcao, opcao.rotulo, opcao.preco)} className="p-1.5 text-gray-900 rounded active:scale-95"><Plus className="w-5 h-5" /></button>
                        </div>
                      </div>
                    );
                  })}
          ...
          return (
            <div key={produto.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 flex justify-between items-center gap-4">
              ...
              <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-1 border border-gray-300 shadow-sm">
                <button onClick={() => handleBotaoMenos(produto)} className={`p-2 rounded-lg ${quantidade > 0 ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-400'}`} disabled={quantidade === 0}><Minus className="w-5 h-5" /></button>
                <span className="font-black w-6 text-center text-black text-lg">{quantidade}</span>
                <button onClick={() => handleBotaoMais(produto)} className="p-2 bg-white text-gray-900 rounded-lg shadow-sm active:scale-95 transition-transform"><Plus className="w-5 h-5" /></button>
              </div>
            </div>
          );
        })}
      ...
      {modalCarrinhoAberto && (
        <div className="fixed inset-0 z-50 flex justify-end flex-col bg-gray-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-h-[90vh] rounded-t-3xl shadow-2xl flex flex-col">
            ...
            <div className="p-5 overflow-y-auto flex-1 space-y-8 bg-gray-50">
                      ...
                      <div key={item.uid} className="bg-white p-4 rounded-2xl border-2 border-orange-200 shadow-sm">
                        ...
                        <div className="flex flex-col gap-2 mt-2">
                          <div className="flex items-start gap-2 bg-gray-50 rounded-lg p-2 border border-gray-200">
                            ...
                            <textarea placeholder="Observações adicionais..." className="w-full text-sm font-medium outline-none text-gray-800 bg-transparent resize-none h-10" value={item.observacao} onChange={(e) => atualizarObservacaoUID(item.uid, e.target.value)} />
                          </div>
                      ...
                      <div key={item.uid} className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm flex flex-col gap-3">
                        ...
                        <div className="flex justify-between items-center border-t border-gray-100 pt-3">
                          ...
                          <button onClick={() => { if (window.confirm(`Cancelar ${item.nome}?`)) removerItemInteiroComanda(item.uid); }} className="text-red-600 bg-red-50 hover:bg-red-100 p-2 rounded-lg border border-red-200 font-bold text-sm flex items-center gap-1.5">
                            <Trash2 className="w-4 h-4" /> Cancelar Item
                          </button>
                        </div>
                      </div>
                    ))}
              ...
              {comandaDaMesaAtiva.length > 0 && carrinho.length === 0 && (
                <div className="pt-4 border-t border-gray-200 mt-6">
                  <button onClick={() => setModalContaAberto(true)} className="w-full bg-gray-900 text-white font-black text-lg py-4 rounded-xl flex items-center justify-center gap-2 shadow-lg active:scale-[0.98] transition-transform">
      ...
      {!modalCarrinhoAberto && !modalAcomp && totalGeral > 0 && (
        <div className="fixed bottom-6 left-6 right-6 z-20">
          <button onClick={() => setModalCarrinhoAberto(true)} className={`w-full text-white rounded-2xl p-4 flex items-center justify-between shadow-xl active:scale-[0.98] transition-all ${qtdItensCarrinho > 0 ? 'bg-orange-600' : 'bg-gray-900'}`}>
            <div className="flex items-center gap-3">
              <div className="bg-white/20 p-2 rounded-xl relative">
                <ShoppingCart className="w-5 h-5" />
                {qtdItensCarrinho > 0 && (
                  <span className="absolute -top-2 -right-2 bg-yellow-400 text-gray-900 text-xs font-black w-5 h-5 rounded-full flex items-center justify-center">{qtdItensCarrinho}</span>
                )}
              </div>
      ...
      {modalContaAberto && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-md max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="p-5 bg-gray-50 border-t border-gray-200 space-y-4">
              ...
              <div className="flex justify-between items-center text-gray-900 bg-white p-3 rounded-xl border border-gray-200 shadow-sm cursor-pointer select-none active:scale-[0.98] transition-transform" onClick={() => setIncluirTaxa(!incluirTaxa)}>
                <div className="flex items-center gap-3">
              ...
              <div className="pt-2">
                ...
                <select value={formaPagamentoAtual} onChange={(e) => setFormaPagamentoAtual(e.target.value)} className="w-full bg-white border border-gray-300 text-gray-900 font-bold p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 shadow-sm">
              ...
              <button onClick={confirmarFechamentoConta} className="w-full mt-2 bg-orange-600 text-white font-black text-lg py-4 rounded-xl shadow-lg hover:bg-orange-500 active:scale-[0.98] transition-transform">
                Imprimir e Encerrar
              </button>
            </div>
          </div>
